...
description: Audit a web UI for compliance against Vercel's web-interface-guidelines — interactions, animation, layout, content, forms, performance, accessibility, and copy. Use as a critical compliance pass over rendered output (complements design-review).
---

> **How to use.** Render the page, then check it against every applicable item below. For each miss, report **what · where · the fix**. Many items are app-specific (hydration, optimistic UI, virtualization) — skip what does not apply to a static marketing site. Vendored from Vercel; see SOURCE.md.
